...
```{r}
taxons_insect <- read.csv("/genetics/edna/workdir/bottomtrawl_2022/20230310/trimmed/filtered2/outputs/asv_taxonomy_insect.csv", row.names = 1) %>%
...
```{r}
asv_table <- read.csv("/genetics/edna/workdir/bottomtrawl_2022/20230310/trimmed/filtered2/outputs/ASVtable.csv") %>%
  rename(SampleID = X)
```
...
read_summary_long <- read_summary %>%
  pivot_longer(cols = 2:28, names_to = "taxon", values_to = "reads") %>%
