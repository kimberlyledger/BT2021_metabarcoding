...
```{r}
taxons_blast <- read.csv("/genetics/edna/workdir/bottomtrawl_2022/20230216/trimmed/filtered/outputs/asv_taxonomy_blastn.csv", row.names = 1) %>%
...
```{r}
taxons_insect <- read.csv("/genetics/edna/workdir/bottomtrawl_2022/20230216/trimmed/filtered/outputs/asv_taxonomy_insect.csv", row.names = 1) %>%
...
```{r}
asv_table <- read.csv("/genetics/edna/workdir/bottomtrawl_2022/20230216/trimmed/filtered/outputs/ASVtable.csv") %>%
  rename(SampleID = X)
```
...
read_summary_long <- read_summary %>%
  pivot_longer(cols = 2:23, names_to = "taxon", values_to = "reads") %>%
...
okay, everything looks reasonable here except for the field blank for sample 92.
