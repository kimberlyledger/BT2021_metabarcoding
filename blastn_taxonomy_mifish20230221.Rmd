...
taxonomy <-read.delim("/genetics/edna/workdir/bottomtrawl_2022/20230221/trimmed/filtered/outputs/blastn_taxlineage.txt", header = FALSE, na.strings=c(""," ","NA"))
...
only 24 identifiable ASV's for the dataset seems low... 
...
only hybrid seq is O.mykiss and Salmo salar... will just remove these ID's since other Oncorhynchus ID's are present
```{r}
taxon_df <- taxon_df %>%
...
single_spp_seqs <- taxon_df %>% 
  ...
  filter(n_taxa == 1)
```
ZERO sequences are single-species
...
```{r}
write.csv(collapsed_taxonomy, "/genetics/edna/workdir/bottomtrawl_2022/20230221/trimmed/filtered/outputs/asv_taxonomy_blastn.csv")
```
